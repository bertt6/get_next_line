...
char	*ft_strjoin(char *lft_str, char *buff)
{
	...
	if (!lft_str)
	{
		lft_str = calloc(sizeof(char) , 1);
		lft_str[0] = '\0';
	}
	if (!buff)
		return (0);
	str = calloc(sizeof(char) , (ft_strlen(lft_str) + ft_strlen(buff) + 1));
	if (!str)
		return (0);
	i = -1;
	while (lft_str[++i] != '\0')
...
char *ft_clean(char *handle_string)
{
    ...
    ptr = calloc (sizeof(char) , (ft_strlen(handle_string) - i + 1));
    i++;
